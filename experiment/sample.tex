...
\hfill Date: 01/01/2025 \\
\noindent
\textbf{Title:} Implementation of secure socket communication using AES block cipher.

\subsection*{Code}
% \begin{lstlisting}[style=mypython, caption={.py}, label={code:A-1-a-}]
% \end{lstlisting}

\lstinputlisting[style=mypython, caption={Encryption.py}]{script/Encryption.py}

\begin{lstlisting}[style=mypython, caption={TerminalMessage.py}, label={code:A-1-a-TerminalMessgae}]
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{assets/report/assignment-01/a/output.png}
    \caption{Secure socket communication b/w server and client using AES block ciper.}
